...
def parse_markdown(text: str, ctx: LineContext = None) -> list[TextSegment]:
    ...
    if re.match(r'^[\s\-*_]{3,}\s*$', text.strip()):
        # 如果在表格中，结束表格
        if ctx.in_table:
            table_text = _serialize_table(ctx)
            ctx.in_table = False
            ctx.table_rows.clear()
            ctx.table_header_parsed = False
            return [TextSegment(text=table_text, no_wrap=True)]
    ...
    if ctx.in_table:
        ...
        # 在表格和当前行之间插入换行
        table_seg = TextSegment(text=table_text, no_wrap=True)
        if segments:
            return [table_seg, TextSegment(text="", horizontal_rule=False)] + segments
        return [table_seg]
...
def _serialize_table(ctx: LineContext) -> str:
    """将表格序列化为文本（带特殊标记）"""
